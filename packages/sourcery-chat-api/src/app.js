...
import { getConversation } from "@sourcery/frontend/src/lib/classes/conversations.ts";
...
httpServer.post("/chat/:project_id", async (req, res) => {
    try {
        ...
        // Get the context
        const context = await get_rag_context(project_id, files_to_search, input, top_k, vector_model);
        ...
        const chatStream = await ollama.chat({ model, messages, stream: true, options: { temperature } }).catch(err => {
            console.error(err);
            throw new restifyErrors.InternalServerError("Error processing chat");
        });
        const response_log = [];
